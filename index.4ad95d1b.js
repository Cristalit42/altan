...
const $a6ebff98972dc7c5$var$paginationItems = document.querySelectorAll('.pagination-item');
const $a6ebff98972dc7c5$var$slideDuration = 4000;
let $a6ebff98972dc7c5$var$currentIndex = 0;
let $a6ebff98972dc7c5$var$timer;
function $a6ebff98972dc7c5$var$resetProgressBars() {
    $a6ebff98972dc7c5$var$progressBars.forEach((bar, i)=>{
        bar.style.transition = 'none';
        bar.style.width = i < $a6ebff98972dc7c5$var$currentIndex ? '100%' : '0%';
    });
}
function $a6ebff98972dc7c5$var$animateProgressBar(index) {
    $a6ebff98972dc7c5$var$resetProgressBars();
    const bar = $a6ebff98972dc7c5$var$progressBars[index];
    if (!bar) return;
    bar.style.transition = 'none';
    bar.style.width = '0%';
    requestAnimationFrame(()=>{
        bar.style.transition = `width ${$a6ebff98972dc7c5$var$slideDuration}ms linear`;
        bar.style.width = '100%';
    });
}
function $a6ebff98972dc7c5$var$nextSlide() {
    const prevIndex = $a6ebff98972dc7c5$var$currentIndex;
    $a6ebff98972dc7c5$var$currentIndex = ($a6ebff98972dc7c5$var$currentIndex + 1) % $a6ebff98972dc7c5$var$progressBars.length;
    $a6ebff98972dc7c5$var$animateTextTransition(prevIndex, $a6ebff98972dc7c5$var$currentIndex); // 👈 добавлено
    $a6ebff98972dc7c5$var$slider.slideToLoop($a6ebff98972dc7c5$var$currentIndex);
    $a6ebff98972dc7c5$var$scrollToActivePagination($a6ebff98972dc7c5$var$currentIndex);
}
function $a6ebff98972dc7c5$var$startTimer() {
    $a6ebff98972dc7c5$var$animateProgressBar($a6ebff98972dc7c5$var$currentIndex);
    $a6ebff98972dc7c5$var$applyZoomEffect($a6ebff98972dc7c5$var$currentIndex); // запуск зума на активном слайде
    $a6ebff98972dc7c5$var$timer = setTimeout(()=>{
        $a6ebff98972dc7c5$var$nextSlide();
        $a6ebff98972dc7c5$var$startTimer();
    }, $a6ebff98972dc7c5$var$slideDuration);
}
function $a6ebff98972dc7c5$var$stopTimer() {
    clearTimeout($a6ebff98972dc7c5$var$timer);
}
// Новая версия с проверкой ширины
function $a6ebff98972dc7c5$var$scrollToActivePagination(index) {
    if (window.innerWidth >= 900) // Если ширина >= 900, не скроллим пагинацию
    return;
    const activeItem = $a6ebff98972dc7c5$var$paginationItems[index];
    const container = document.querySelector('.main-section__swiper-pagination');
...
// при изменении слайда вручную
$a6ebff98972dc7c5$var$slider.on('slideChange', ()=>{
    $a6ebff98972dc7c5$var$stopTimer();
    $a6ebff98972dc7c5$var$currentIndex = $a6ebff98972dc7c5$var$slider.realIndex % $a6ebff98972dc7c5$var$progressBars.length;
    $a6ebff98972dc7c5$var$scrollToActivePagination($a6ebff98972dc7c5$var$currentIndex);
    $a6ebff98972dc7c5$var$startTimer();
});
// при клике по кастомной пагинации
// paginationItems.forEach((item, index) => {
//   item.addEventListener('click', () => {
//     stopTimer();
//     animateTextTransition(currentIndex, index); // 👈 добавлено
//     currentIndex = index;
//     scrollToActivePagination(currentIndex);
//     slider.slideToLoop(index);
//     startTimer();
//   });
// });
function $a6ebff98972dc7c5$var$applyZoomEffect(index) {
    const slides = document.querySelectorAll('.main-section__swiper-slide');
    slides.forEach((slide, i)=>{
        slide.classList.remove('zooming');
        if (i === index) {
            // перезапускаем анимацию
            slide.offsetWidth; // принудительный reflow для перезапуска анимации
            slide.classList.add('zooming');
        }
    });
}
function $a6ebff98972dc7c5$var$animateTextTransition(prevIndex, nextIndex) {
    const slides = document.querySelectorAll('.main-section__swiper-slide');
    const prevSlide = slides[prevIndex];
    const nextSlide = slides[nextIndex];
    const prevText = prevSlide?.querySelector('.main-section__slide-desscr');
    const nextText = nextSlide?.querySelector('.main-section__slide-desscr');
    if (prevText) {
        prevText.classList.remove('animate-in');
        prevText.classList.add('animate-out');
    }
    if (nextText) {
        nextText.classList.remove('animate-out');
        // задержка для плавного входа после выхода предыдущего
        setTimeout(()=>{
            nextText.classList.add('animate-in');
        }, 100);
    }
}
// Запуск при загрузке
